...
int main(int argc, char *argv[])
{
	...
	gtk_init(&argc, &argv); 	
 	...
 	label = gtk_label_new("Please choose");
	box_1 = gtk_box_new(GTK_ORIENTATION_VERTICAL, 20); 	
 	box_2 = gtk_button_box_new(GTK_ORIENTATION_HORIZONTAL);
	...
	gtk_button_box_set_layout(GTK_BUTTON_BOX(box_2), GTK_BUTTONBOX_SPREAD);	
	...
	gtk_box_pack_start(GTK_BOX(box_1), label, FALSE, FALSE, 2);	
	gtk_box_pack_start(GTK_BOX(box_1), box_2, FALSE, FALSE, 2);
	
	gtk_container_add(GTK_CONTAINER(box_2), cancel_b);
	gtk_container_add(GTK_CONTAINER(box_2), logout_b);
	gtk_container_add(GTK_CONTAINER(box_2), restart_b);	
	gtk_container_add(GTK_CONTAINER(box_2), shutdown_b);
